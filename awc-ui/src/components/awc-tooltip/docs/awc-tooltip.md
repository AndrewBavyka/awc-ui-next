...
| `position`   | `position`    | `"top" \| "bottom" \| "left" \| "right"` | "top"      | Позиция тултипа относительно элемента.           |
| `spacing`    | `spacing`     | `AwcTooltipSpacing`                      | "8"        | Расстояние в пикселях между тултипом и элементом. |
| `strategy`   | `strategy`    | `"absolute" \| "fixed"`                  | "absolute" | Стратегия позиционирования тултипа (absolute или fixed). |
